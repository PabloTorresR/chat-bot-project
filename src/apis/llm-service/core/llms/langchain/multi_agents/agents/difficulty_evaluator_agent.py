...
from core.llms.langchain.templates.system.vocabulary_generation.difficulty_evaluator import (
    template,
)
...
class DifficultyEvaluatorAgent:
    ...
    async def generate(self, words, language, native_language) -> dict:
        ...
        structured_llm = self.llm.with_structured_output(Difficulty)
        return await structured_llm.ainvoke(chat_prompt)

    async def run(self, state):
        difficulty: dict = await self.generate(
            words=state["words"],
