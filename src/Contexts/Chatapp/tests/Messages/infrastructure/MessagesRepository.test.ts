...
import { DynamoMessageRepository } from './../../../../../Contexts/Chatapp/Messages/infrastructure/persistence/dynamodb/DynamoDBMessageRepository';
import { DynamoDBClientFactory } from './../../../../../Contexts/Shared/infrastructure/persistence/dynamodb/DynamoDBClientFactory';
import { DynamoConfigFactory } from '../../../Shared/infrastructure/persistence/dynamodb/DynamoDBConfigFactory';
...
describe('MessagesCounterRepository', () => {
  describe('#save', () => {
    it('should save a messages counter', async () => {
      const config = await DynamoConfigFactory.createConfig();

      const dbClient = DynamoDBClientFactory.createClient('chatapp', config);
      const repository = new DynamoMessageRepository(dbClient);
