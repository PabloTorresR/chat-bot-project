...
const queues: QueueConfig[] = Object.values(subscribers).map(subscriber => ({
  name: subscriber._Object().name.replaceAll('.', '-'),
...
async function main(): Promise<void> {
  const eventBusName = 'chatapp.domain_events';
  ...
  targets.forEach(target => {
    writeTerraformFile(`event_target_${target.ruleName}`, generateEventBridgeTargetTerraform(target));
  });
}
